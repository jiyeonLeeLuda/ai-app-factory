...
4. **응답 스트리밍**: 아티스트 말풍선이 왼쪽에 빈 채로 생기고, 답이 **문장 단위 말풍선으로 하나씩** 뜬다(서버는 SSE 스트리밍, 화면은 문장 말풍선 — 상세 아래 "S1 진화(B)"가 본 규격). 원안은 토큰 타이핑이었으나 실측 후 B로 확정. 스트리밍 중에는 "생각 중" 커서/점멸 표시.
5. **중단**: 스트리밍 중 "중지" 버튼을 누르면 그 시점까지 받은 텍스트로 말풍선이 확정된다.
6. **영속**: 새로고침해도 대화가 그대로 남아 있다(쿠키의 `userId` + 아티스트로 방을 복원).

### S1 진화 (완료 후, B안 — 2026-09-16)

실사용에서 격차 가설 수정: 팬-아티스트 채팅의 진짜 장면은 "토큰 타이핑"이 아니라 **문장 단위 다중 말풍선**(디어유 버블류)이었다. **SSE 스트리밍은 서버에 유지**(격차 성립)하되, 화면 표현을 바꾼다:
- 서버가 스트림을 **문장 경계로 잘라** `sentence` 이벤트로 emit, **문장 하나당 assistant `Message` 1행** 저장(새로고침 시에도 다중 말풍선 유지).
- 클라는 문장을 큐에 받아 **약간의 시차(~600ms)**로 하나씩 말풍선 노출, 사이사이 **"..." 순차 점멸** 타이핑 표시(S1의 "생각중 표시" 구현).
- 중단 시: 남은 버퍼 flush + 마지막 문장에 `…(사용자가 중단함)` 부착 저장. 실패 시: 저장 없이 에러+재시도.
- **"..." 타이핑은 전송 직후부터**: 스트리밍 시작(전송 즉시)~완료까지 아티스트 프로필 옆 맨 아래에 항상 표시(첫 문장 대기 중에도). 큐 비고 done 되면 소멸.
- **분할 기준**: 문장부호(`. ! ? 。 ~ …`) **또는 줄바꿈**. 반말이라 마침표가 없을 수 있으니 페르소나가 줄바꿈으로 마디를 나눈다(S2b).
- **다중 턴 안전(중요, 실측 버그)**: 매 전송은 독립 처리. **이전 턴의 큐·타이머·done 플래그를 완전히 초기화**해, 2번째 응답부터 말풍선이 안 뜨거나 사라지는 일이 없게 한다. (P1 hand-B에서 2턴째 blank 버그 발생 — 재실행 시 이 엣지 반드시 통과.)
...
- **톤**: 다정하고 나른한 반말·존댓말 중간체. 팬을 편하게 대하되 과하게 들뜨지 않음. 이모지는 한 메시지에 최대 1개. **답변은 짧은 문장으로 끊어 쓰고 여러 마디는 줄바꿈으로 나눈다** — 문장 단위 말풍선 분할(S1 B안)이 살아나도록. (반말이라 마침표가 없어도 줄바꿈이 분할 기준이 됨)

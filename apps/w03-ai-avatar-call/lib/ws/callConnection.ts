...
import { sanitizeForTTS } from "../voice/emotion";

// 연결별 통화 세션 핸들러 (w3-spec S6 lib/ws).
// - 턴 히스토리는 이 연결이 살아있는 동안만 메모리 배열로 유지 → 끊기면 소멸(휘발, S2).
// - 통화 내용은 DB 미저장. 개발 로그로만 전사·응답·emotion을 stderr에 남긴다(S4).

const GREETING = "여보세요?"; // 첫 턴 = 아티스트 선턴 (사용자 입력 없이, S1.4)
const GREETING_EMOTION = "relaxed"; // 선턴 표정 (S1.4)
...
export async function handleCallConnection(ws: WebSocket, req: IncomingMessage) {
  ...
  // 한 발화 = STT → LLM(선두 emotion + 문장 스트리밍) → emotion 전송 + 문장마다 sanitize→TTS → 순서대로 전송
  async function handleUtterance(audioIn: Buffer) {
    ...
    let full = "";
    let emotion = "neutral";
    for await (const chunk of streamCall(instructions, history)) {
      if (closed) return;
      if (chunk.type === "emotion") {
        emotion = chunk.value;
        sendJSON({ type: "emotion", value: chunk.value }); // 표정 lerp 즉시 시작 (격차 ②)
        continue;
      }
      // 문장 → TTS 직전 stray [[...]] 제거 (TTS가 대괄호 안 읽게, S3 G1)
    ...
    console.error(
      `[call ${artist!.slug}] [emotion] ${emotion} [LLM] ${JSON.stringify(full)}`,
    );
  ...
  try {
    ...
    sendJSON({ type: "emotion", value: GREETING_EMOTION }); // 선턴 표정 (S1.4)
    sendJSON({ type: "gesture", value: "wave" }); // 선턴 = 손 흔들며 인사 (+ 시네마틱 줌아웃, 사용자 요청)

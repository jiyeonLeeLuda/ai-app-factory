...
const SENTENCE_DELAY_MS = 600; // 문장 말풍선 사이 시차 (p1-spec S1b)
...
export default function ChatRoom({
  ...
  // ── 턴별 상태 (매 전송마다 완전 초기화 — 2턴째 blank 버그 방지, p1-spec S1b 다중턴 안전) ──
  ...
  // 실패한 턴의 assistant 말풍선 제거 (실패 답은 DB에 안 남으므로 화면도 정리, p1-spec S3)
  ...
  async function runStream(turnId: number, body: Record<string, unknown>) {
    ...
    try {
      ...
      if (turnRef.current === turnId && !doneRef.current) {
        // AbortError(사용자 중단)는 handleStop에서 처리하므로 여기서는 진짜 실패만

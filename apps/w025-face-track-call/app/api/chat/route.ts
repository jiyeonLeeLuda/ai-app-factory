...
export const runtime = "nodejs"; // Edge 아님 (S4)
...
// (userId, artistId) 방을 멱등하게 확보 — 동시요청에도 @@unique 위반으로 죽지 않게 (S3)
...
export async function POST(request: Request) {
  ...
  // 새 전송이면 user 메시지 생성, 재시도면 이미 저장된 마지막 user 메시지를 재사용 (S3)
  if (body.retry) {
  ...
  // 최근 20턴만 API에 실어 토큰 폭주 방지 (S4). DB가 진실의 원천 — 수동 재전송.
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      // assistant 문장들을 한 행씩 저장 (완료/중단 시에만 — 실패 시 저장 안 함, S3)
      ...
      try {
        ...
        if (aborted) {
          // 사용자 중단: 남은 버퍼 flush + 마지막 문장에 꼬리표 붙여 저장 (S1b/S3)
          const tail = splitter.flush();

...
// "누구냐"는 오직 쿠키의 userId. W4 auth에서 쿠키→진짜 세션 교체를 여기 한 곳만 고친다.
...
export async function setUserCookie(userId: string): Promise<void> {
  ...
  store.set(USER_COOKIE, userId, {
    ...
    path: "/",
    maxAge: ONE_YEAR,
    // 이 앱은 dev·prod 모두 HTTPS(server.ts https)로만 서빙 → 항상 Secure.
    // iOS Safari는 HTTPS에서 Secure 없는 쿠키를 저장 안 해 로그인이 유실됨(2026-09-17 실측).
    secure: true,
  });
}
